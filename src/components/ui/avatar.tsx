...
export function getUserAvatarUrl(user?: User | null) {
  ...
  if (!user?.id) return 'https://api.dicebear.com/9.x/lorelei/svg';

  const dicebear = `https://api.dicebear.com/9.x/lorelei/svg?seed=${encodeURIComponent(user.id)}`;
  // Gravatar default doesn't support SVG or query params, so we use PNG and encoded / params
  const encodedDicebear = `https://api.dicebear.com/9.x/lorelei/png/${encodeURIComponent(`seed=${encodeURIComponent(user.id)}`)}`;
