...
export function Modal({
  ...
  if (isMobile && !desktopOnly) {
    return (
      ...
      >
        <Drawer.Portal>
          <Drawer.Overlay className="fixed inset-0 z-50 bg-neutral-800 bg-opacity-10 backdrop-blur" />
          <Drawer.Content
            className={cn(
              'fixed bottom-0 left-0 right-0 z-50 flex flex-col',
              'rounded-t-[10px] border-t border-border bg-background',
              className,
            )}
          >
    ...
    >
      <Dialog.Portal>
        <Dialog.Overlay
          ...
          id="modal-backdrop"
          className="animate-fade-in fixed inset-0 z-40 bg-neutral-800/40 backdrop-blur-md"
        />
        <Dialog.Content
          ...
          className={cn(
            ...
            'border border-border bg-background p-0 shadow-xl sm:rounded-2xl',
